...
V_w = 0.018; # Volume of vessel structure & internals [m^3] (This should actually be 0.02 m^3)
...
for i in range(1,n):
    ...
    #end
    m = ( m  - m_dot  * delta_t);
    T = ( delta_t * h * As / (c_v * m )*Twtemp  + (1 - delta_t*(h * As + m_dot *R)/(c_v * m ))*Ttemp );
    Tw = (  delta_t * 0.066 * Po *((i)*delta_t + (delta_t **2) **(-0.2))/(rho_w * V_w *c_w) + delta_t * h * As/(rho_w * V_w *c_w)*Ttemp  + ( 1 - delta_t * h * As / (rho_w * V_w *c_w))*Twtemp);
    t = ( t  + delta_t);
    Twtemp = Tw
    Ttemp = T
...
print "max of vg_array =",max(vg_array)
