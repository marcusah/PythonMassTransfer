...
c_pAir = 1.01e3; # Specific Heat capacity under constant P for Air [J/Kg/K]
c_vAir = 718.0; # Specific Heat capacity under constant V for Air[J/Kg/K]


mu_ambHe = 1.96e-5;# ambiant He viscosity [kg/m/s or Pa*s]
mu_ambAir = 1.82e-5;# ambiant Air viscosity [kg/m/s or Pa*s]


########################## For Vessel 1: The PV ########################################

# Gas properties
R_v1 = R_He # 2076.9; # Helium gas constant [J/Kg/K]
c_p_v1 = c_pHe #5190.1; # Specific Heat capacity under constant P for He [J/Kg/K]
c_v_v1 = c_vHe #3122.0; # Specific Heat capacity under constant V_v1 for He[J/Kg/K]
...
# Gas properties
R_v2 = 287.05; # V2 gas constant [J/Kg/K]
c_p_v2 =  1.01e3; # Specific Heat capacity under constant P for V2 [J/Kg/K]
c_v_v2 = 718.0; # Specific Heat capacity under constant V_v1 for V2[J/Kg/K]
c_p_v2temp = c_pAir;
c_v_v2temp = c_vAir;
...
m_i_v2 = rho_i_v2*V_v2;# intial He Mass in vessel [kg]
...
####################### Vessel 1 calculations#########################
    ...
    T_v1 =  delta_t * h * As_v1 / (c_v_v1 * m_v1temp )*Tw_v1temp  + (1 - delta_t*(h * As_v1 + m_dot_v1 *h)/(c_v_v1 * m_v1temp ))*T_v1temp ;
    ...
    m_v2 +=  m_dot_v1 * delta_t ; 
    He_v2= (He_v2 * m_v2temp +  m_dot_v1 * delta_t)/m_v2
    c_p_v2 = c_pAir*(1 - He_v2) + c_pHe*He_v2
    c_v_v2 = c_vAir*(1-He_v2) + c_vHe * He_v2
    R_v2 = R_Air*(1-He_v2) + R_He * He_v2
    # mu_amb_v1 = mu_ambAir*(1-He_v2) + mu_ambHe * He_v2
    
    T_v2 =  delta_t * h * As_v2 / (c_v_v2temp * m_v2temp )*Tw_v2temp  + T_v2temp #*(1 - delta_t*(h * As_v2 )/(c_v_v2 * m_v2temp ))*T_v2temp ;
    delta_u_v2 = delta_u_v2 +  m_v2  * c_p_v2 * (T_v2-T_v2temp);
    Tw_v2 = delta_t * h * As_v2/(rho_w * V_w_v2 *c_w)*T_v2temp  + ( 1 - delta_t * h * As_v2 / (rho_w * V_w_v2 *c_w))*Tw_v2temp;

    mu_v2 =  (mu_ambAir*(1-He_v2) + mu_ambHe * He_v2) * (T_v2/To) ** (0.7);
    rho_v2 = m_v2/V_v2;
...
cp_Data_v2 = [t_array, cp_array_v2]
...
with open('cpV2.csv', 'w') as csvFile:
    writer = csv.writer(csvFile)
    writer.writerows(cp_Data_v2)
