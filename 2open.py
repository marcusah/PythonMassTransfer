...
tol=1e-6;#tolerance
t_max = 60.0;# duration of blowdown transient [s]
...
mu_i_v1 = 0.0364;#initial He viscosity [Pa*s]
rho_i_v1 = 7.767;# intial helium density [kg/m^3]
m_i_v1 = rho_i_v1 * V_v1;# intial He Mass in vessel [kg]
...
mu_i_v2 = mu_ambAir * (Ti_v2/To)**(0.7);#initial He viscosity [Pa*s]
m_i_v2 =  m_i_v1 / 2.0;# intial He Mass in vessel [kg]
...
####################### Vessel 1 calculations#########################
    ...
    T_v2 =  delta_t * h * As_v2 / (c_v_v2 * m_v2temp )*Tw_v2temp  + T_v2temp #*(1 - delta_t*(h * As_v2 + m_dot_v2 *R_v2)/(c_v_v2 * m_v2temp )) ;
    delta_u_v2 +=  m_v2  * c_p_v2 * (T_v2 - T_v2temp);
    Tw_v2 =  delta_t * h * As_v2/(rho_w * V_w_v2 *c_w)*T_v2temp  + Tw_v2temp #* ( 1 - delta_t * h * As_v2 / (rho_w * V_w_v2 *c_w));
    ...
    if p_v2 > p_amb:
        while abs(err_v2) >= tol:
            v_g_v2 =  math.sqrt((p_v2  - p_amb)/(rho_avg_v2*(alpha / 2.0  + 0.5*(K_c + f_g_v2*L_v2/D_v2))));
            if v_g_v2  >= a_v2: 
                Re_g_v2 = (rho_avg_v2 * a_v2 * D_v2/mu_avg_v2);
            else :
                Re_g_v2 = (rho_avg_v2 * v_g_v2  * D_v2/mu_avg_v2);
            #end
            if Re_g_v2  >= 4000:
                f1_v2 =  0.184*Re_g_v2  **(-0.2);
                while abs(err1_v2) >=tol:
                    f_v2 =  (1/(-2*math.log(eps/D_v2)/3.7 + 2.51/(Re_g_v2 *math.sqrt(f1_v2)))) **2;
                    err1_v2 = f1_v2  - f_v2 ;
                    f1_v2  = f_v2 ;
            ...
            #print "t=  ", t 
            v_v2 = math.sqrt((p_v2 - p_amb)/(rho_avg_v2 *(alpha/2.0 + 0.5*(K_c + f_v2 *L_v2/D_v2))));
            err_v2 = abs(v_g_v2  - v_v2) ;
            f_g_v2 = f_v2 ;
    ...
    m_dot_v2 = ( rho_avg_v2 * Ac_v2 * v_v2);
    Re_v2 = ( rho_avg_v2 * v_v2 * D_v2 / mu_avg_v2);
    
    m_v2 +=  ( - m_dot_v2)  * delta_t;
    T_v2 =  delta_t * h * As_v2 / (c_v_v2 * m_v2temp )*Tw_v2temp  + T_v2temp * (1 - delta_t*(  m_dot_v2 *R_v2)/(c_v_v2 * m_v2temp )) ;
#   end
